    ...
    def post(self, request):
        ...
        if (
            ...
            try:
                ...
                name_tag = request.data["name"] + number_id
...
class PasswordRecoverList(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {"message": "Email no existe"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def VerifyUser(request, verification_secret):
    try:
        ...
        return Response(
            {"message": "Unable to verify email"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        if list_type == "multimedia":
            try:
                ...
                publication_list = Publication.objects.filter(
                    Q(creator=user),
                    Q(publication_pick__isnull=False),
                    ~Q(publication_pick=""),
                ).order_by("-creation_date")[:20]
                serializer = PublicationInformationSerializer(
                    publication_list, context={"owner": user}, many=True
